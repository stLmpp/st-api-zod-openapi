...
import { generateSchema, extendApi } from './zod-openapi.js';
    ...
    const zodSchema = extendApi(
      z
        .object({
          ...
          aStringNonEmpty: z.string().min(1),
        })
        .partial(),
      {
    ...
    expect(schemaTest).toEqual({
      ...
      properties: {
        ...
        literals: {
          type: 'object',
          properties: {
            wordOne: { type: 'string', enum: ['One'], nullable: true },
            numberTwo: { type: 'number', enum: [2] },
            isThisTheEnd: { type: 'boolean', enum: [false], nullable: true },
          },
          required: ['wordOne'],
        },
        catchall: {
        ...
        aNullish: { type: 'string', nullable: true },
